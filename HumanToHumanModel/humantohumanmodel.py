...
psicosisOff = 0.02 #Infected population percentage threshold which desactivates psicosis factor
...
startCaseCount = 5 #Number of confirmed cases needed to start government actions
...
testingResponse = 0.05 #The probability for a symptomatic human of being tested by the government each day
...
for i in range(simulationsCount):
	...
	s = sim(simulationsPopulation, simulationsPeriod, i + 1, casesCeroCount, simulationName, \
		runGovActions, govActions, govFailureList, autoIsolationThreshold, psicosis, psicosisTrigger, \
